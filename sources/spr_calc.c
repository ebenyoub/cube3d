...
void    spr_swap(all_t *a)
{
	...
	{
		if (a->s.spr_dist[a->s.order[i]] <
				a->s.spr_dist[a->s.order[i + 1]])
		{
			tmp = a->s.order[i];
    ...
    while (a->m.map_tab[++i])
    {
        ...
        while (a->m.map_tab[i][++w])
        {
            if (a->m.map_tab[i][w] >= '2' && a->m.map_tab[i][w] <= '6')
            {
                ...
                a->d[x].y = w + 0.5;
                a->s.order[x] = x;
                a->s.spr_dist[x] = ((a->r.posX - a->d[x].x) * (a->r.posX - a->d[x].x)
                    + (a->r.posY - a->d[x].y) * (a->r.posY - a->d[x].y));
                x++;
            }
        }
    }
...
void	ft_get_color(int i, all_t *a)
{
	...
	if (a->s.texY > 0)
	{
		if (a->p.img_data[a->s.texX + a->s.texY
				* a->p.spr_width] != 0)
			a->m.w = a->p.img_data [a->s.texX + a->s.texY * a->p.spr_width];
		else
			a->m.w = 0xBFD195;
	}
}
    ...
    while (++i < a->s.spr_nbr)
    {
        ...
        while (z < a->s.drawEndX)
        {
            a->s.texX =  (int)(256 * (z - (-a->s.spriteWidth / 2 +
                a->s.spriteScreenX)) * a->p.spr_width / a->s.spriteWidth) / 256;
            if(a->s.transformY > 0 && z > 0 && z < a->m.width && a->s.transformY < a->s.zbuffer[z])
            {
                y = a->s.drawStartY + 1;
                while (y < a->s.drawEndY) 
                {
                    d = y * 256 - a->m.height * 128 + a->s.spriteHeight * 128;
                    a->s.texY = ((d * a->p.spr_height) / a->s.spriteHeight) / 256;
